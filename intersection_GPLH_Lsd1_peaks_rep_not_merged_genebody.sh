...
grep -v "^#" $ref_annot |grep -P "\tgene\t"| sort -k1,1 -k4,4n >${output_dir}all_gene_body.gff
...
${bedtools} closest -a ${output_dir}all_gene_body.gff -b ${G_merged_rep} -t first -D a -sorted -nonamecheck >${output_dir}G_closest_feat.bed || { echo "closestbed  failure 1 !" 1>&2; exit; }
...
awk 'OFS="\t"{split($9,a,";");b="";for(i=1;i<=length(a);i++){if(a[i]~/gene_name/){b=a[i]}};print $1,$4,$7,b,$NF,$(NF-1)}' ${output_dir}G_closest_feat.bed |sed 's/gene_name=//g'|awk 'function abs(v){return v < 0 ? -v : v}{OFS="\t";if($5==0){print}else{if(abs($5)<=0.000){print}}}' |awk '{if($NF~/G1/ && $NF~/G2/){print}}' |sort -u -k4,4|cat <(echo -e "chromosome\ttranscript_start\ttranscript_strand\tgene_name\tdist_peak_to_transcript_loc\tpeaks_group") - >${output_dir}G_genes_with_peaks_in_both_rep.tsv

awk 'OFS="\t"{split($9,a,";");b="";for(i=1;i<=length(a);i++){if(a[i]~/gene_name/){b=a[i]}};print $1,$4,$7,b,$NF,$(NF-1)}' ${output_dir}G_closest_feat.bed |sed 's/gene_name=//g'|awk 'function abs(v){return v < 0 ? -v : v}{OFS="\t";if($5==0){print}else{if(abs($5)<=0.000){print}}}' |awk '{if(($NF~/G1/ && $NF!~/G2/) || ($NF~/G2/ && $NF!~/G1/)){print}}' |sort -u -k4,4|cat <(echo -e "chromosome\ttranscript_start\ttranscript_strand\tgene_name\tdist_peak_to_transcript_loc\tpeaks_group") - >${output_dir}G_genes_with_peaks_in_just_one_rep.tsv 
...
${bedtools} closest -a ${output_dir}all_gene_body.gff -b ${L_merged_rep} -t first -D a -sorted -nonamecheck >${output_dir}L_closest_feat.bed || { echo "closestbed  failure 1 !" 1>&2; exit; }
...
awk 'OFS="\t"{split($9,a,";");b="";for(i=1;i<=length(a);i++){if(a[i]~/gene_name/){b=a[i]}};print $1,$4,$7,b,$NF,$(NF-1)}' ${output_dir}L_closest_feat.bed |sed 's/gene_name=//g'|awk 'function abs(v){return v < 0 ? -v : v}{OFS="\t";if($5==0){print}else{if(abs($5)<=0.000){print}}}' |awk '{if($NF~/L1/ && $NF~/L2/){print}}' |sort -u -k4,4|cat <(echo -e "chromosome\ttranscript_start\ttranscript_strand\tgene_name\tdist_peak_to_transcript_loc\tpeaks_group") - >${output_dir}L_genes_with_peaks_in_both_rep.tsv

awk 'OFS="\t"{split($9,a,";");b="";for(i=1;i<=length(a);i++){if(a[i]~/gene_name/){b=a[i]}};print $1,$4,$7,b,$NF,$(NF-1)}' ${output_dir}L_closest_feat.bed |sed 's/gene_name=//g'|awk 'function abs(v){return v < 0 ? -v : v}{OFS="\t";if($5==0){print}else{if(abs($5)<=0.000){print}}}' |awk '{if(($NF~/L1/ && $NF!~/L2/) || ($NF~/L2/ && $NF!~/L1/)){print}}' |sort -u -k4,4|cat <(echo -e "chromosome\ttranscript_start\ttranscript_strand\tgene_name\tdist_peak_to_transcript_loc\tpeaks_group") - >${output_dir}L_genes_with_peaks_in_just_one_rep.tsv 

############################


##look for closest H peak to TSS 0kb/2kb
${bedtools} closest -a ${output_dir}all_gene_body.gff -b ${H_merged_rep} -t first -D a -sorted -nonamecheck >${output_dir}H_closest_feat.bed || { echo "closestbed  failure 1 !" 1>&2; exit; }
...
awk 'OFS="\t"{split($9,a,";");b="";for(i=1;i<=length(a);i++){if(a[i]~/gene_name/){b=a[i]}};print $1,$4,$7,b,$NF,$(NF-1)}' ${output_dir}H_closest_feat.bed |sed 's/gene_name=//g'|awk 'function abs(v){return v < 0 ? -v : v}{OFS="\t";if($5==0){print}else{if(abs($5)<=0.000){print}}}' |awk '{if($NF~/H1/ && $NF~/H2/){print}}' |sort -u -k4,4|cat <(echo -e "chromosome\ttranscript_start\ttranscript_strand\tgene_name\tdist_peak_to_transcript_loc\tpeaks_group") - >${output_dir}H_genes_with_peaks_in_both_rep.tsv

awk 'OFS="\t"{split($9,a,";");b="";for(i=1;i<=length(a);i++){if(a[i]~/gene_name/){b=a[i]}};print $1,$4,$7,b,$NF,$(NF-1)}' ${output_dir}H_closest_feat.bed |sed 's/gene_name=//g'|awk 'function abs(v){return v < 0 ? -v : v}{OFS="\t";if($5==0){print}else{if(abs($5)<=0.000){print}}}' |awk '{if(($NF~/H1/ && $NF!~/H2/) || ($NF~/H2/ && $NF!~/H1/)){print}}' |sort -u -k4,4|cat <(echo -e "chromosome\ttranscript_start\ttranscript_strand\tgene_name\tdist_peak_to_transcript_loc\tpeaks_group") - >${output_dir}H_genes_with_peaks_in_just_one_rep.tsv

##############################

##look for closest P peak to TSS 0kb/2kb
${bedtools} closest -a ${output_dir}all_gene_body.gff -b ${P_merged_rep} -t first -D a -sorted -nonamecheck >${output_dir}P_closest_feat.bed || { echo "closestbed  failure 1 !" 1>&2; exit; }
...
awk 'OFS="\t"{split($9,a,";");b="";for(i=1;i<=length(a);i++){if(a[i]~/gene_name/){b=a[i]}};print $1,$4,$7,b,$NF,$(NF-1)}' ${output_dir}P_closest_feat.bed |sed 's/gene_name=//g'|awk 'function abs(v){return v < 0 ? -v : v}{OFS="\t";if($5==0){print}else{if(abs($5)<=0.000){print}}}' |awk '{if($NF~/P1/ && $NF~/P2/){print}}' |sort -u -k4,4|cat <(echo -e "chromosome\ttranscript_start\ttranscript_strand\tgene_name\tdist_peak_to_transcript_loc\tpeaks_group") - >${output_dir}P_genes_with_peaks_in_both_rep.tsv

awk 'OFS="\t"{split($9,a,";");b="";for(i=1;i<=length(a);i++){if(a[i]~/gene_name/){b=a[i]}};print $1,$4,$7,b,$NF,$(NF-1)}' ${output_dir}P_closest_feat.bed |sed 's/gene_name=//g'|awk 'function abs(v){return v < 0 ? -v : v}{OFS="\t";if($5==0){print}else{if(abs($5)<=0.000){print}}}' |awk '{if(($NF~/P1/ && $NF!~/P2/) || ($NF~/P2/ && $NF!~/P1/)){print}}' |sort -u -k4,4|cat <(echo -e "chromosome\ttranscript_start\ttranscript_strand\tgene_name\tdist_peak_to_transcript_loc\tpeaks_group") - >${output_dir}P_genes_with_peaks_in_just_one_rep.tsv
